...
function mousemove(d) {
    var s = d3.select(this).select( "rect.selection");
    ...
    if( !s.empty()) {
        ...
        d3.selectAll( 'circle.dot.selected').classed( "selected", false);
        var r = 3;
        d3.selectAll( 'circle.dot').each( function(data, i) {
            if( 
                !d3.select( this).classed( "selected") && 
                    // inner circle inside selection frame
                plot.x(data)-r>=d.x && plot.x(data)+r<=d.x+d.width && 
                plot.y(data)-r>=d.y && plot.y(data)+r<=d.y+d.height
            ) {
...
function mouseup() {
    ...
    d3.select(this).selectAll( "rect.selection").remove();

    // remove temporary selection marker class
    d3.selectAll( 'g.state.selection').classed( "selection", false);
    window.dragsel = false;
}
...
function showPtinfo(d) {
    ...
    table.append("thead").append("tr")
        ...
        .enter().append("th")
        .append("a").attr("href", function(d, j) {
            return (j < 2) ? 'javascript:void(0)' : 'javascript:plotD(0,' + (j - 2) + ')'
        })
        .text(function(d) {
            return d;
        });
...
function plotPt(data) {
    ...
    bar.append("svg:rect")
        ...
        .attr("height", function(d) {
            return highlights.indexOf(d.name) < 0 ? 3 : 9
        })
        .attr("width", function(d) {
            return highlights.indexOf(d.name) < 0 ? 3 : 9
        })
        .attr("fill", function(d) {
            return highlights.indexOf(d.name) < 0 ? "#ccc" : "#f00"
        })
        .on("mouseover", function(d) {
            tooltip.html(d.name)
            return tooltip.style("visibility", "visible");
...
function heatmapImpl() {
    ...
    var e = parseInt(d.e)
    var p = e - s,
        n = parseInt((p - p / scale) / 2)
    s += n
    e -= n
    p = p / scale
    var w = Math.max(W / p, 1)
    var start = d.start - p * 1000,
        end = d.end + p * 1000
...
function showinfo(d, i) {
    ...
    table.append("thead").append("tr")
        ...
        .enter().append("th")
        .append("a").attr("href", function(d, j) {
            return (j < 2) ? 'javascript:void(0)' : 'javascript:plotD(' + i + ',' + (j - 2) + ')'
        })
        .text(function(d) {
            return d;
        });
...
function setPCs(d, l) {
    ...
    for (var i in d) {
        ...
        if (d[i].name.indexOf('_') >= 0) {
            s.push({
                i: parseInt(i),
                seqnames: d[i].seqnames,
                start: d[i].start - l,
                end: d[i].end + l,
                id: trimstr(d[i].name, 2)
            })
